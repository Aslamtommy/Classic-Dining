...
const AdminDashboard: React.FC = () => {
  ...
  const [recipientType, setRecipientType] = useState<'restaurant' | 'branch' | 'user'>('restaurant');
  ...
  const handleCustomDateSubmit = (e: React.FormEvent) => {
    ...
    else toast.error('Please select both start and end dates');
  };
  ...
  const handleSendNotification = async (e: React.FormEvent) => {
    ...
    try {
      await adminApi.post('/notifications', { message: notificationMessage, recipientType });
  ...
  if (!data) {
    return <div className="flex items-center justify-center h-screen text-red-500 text-lg">No data available</div>;
  }
    ...
    <div className="min-h-screen bg-gray-50 p-6 font-playfair">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-8">
          ...
          <form onSubmit={handleSendNotification} className="flex flex-col sm:flex-row gap-4">
            <select
              value={recipientType}
              onChange={(e) => setRecipientType(e.target.value as 'restaurant' | 'branch' | 'user')}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-8">
          ...
          {data.restaurantPerformance.map((rest) => (
            <div key={rest.restaurantId} className="p-6 border-b border-gray-200 last:border-b-0">
              <button
                onClick={() => toggleRestaurant(rest.restaurantId)}
                ...
                <div className="mt-4">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <span className="font-medium">Total Revenue:</span> ₹{rest.totalRevenue.toLocaleString()}
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <span className="font-medium">Reservations:</span> {rest.totalReservations}
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <span className="font-medium">Avg Revenue/Reservation:</span> ₹
                      {rest.avgRevenuePerReservation.toFixed(2)}
                    </div>
                  </div>
                  <div className="h-64 mb-4">
                    <Line
                      data={restaurantTrends(rest)}
                      options={withTitle<'line'>(baseLineOptions, `${rest.name} Revenue Trends`)}
                    />
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <ul className="space-y-2">
              {data.topCustomers.map((c) => (
                <li key={c._id} className="text-gray-700">
                  {c.name} ({c.email}) - {c.totalBookings} bookings, ₹{c.totalSpent.toLocaleString()}
                </li>
              ))}
            </ul>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            ...
            <h3 className="text-md font-medium text-gray-700 mt-4">Coupon Usage</h3>
            <ul className="space-y-2">
              {data.systemHealth.couponUsage.map((c) => (
                <li key={c.code} className="text-gray-700">
                  {c.code}: Used {c.timesUsed} times, ₹{c.totalDiscount.toLocaleString()} discounted
                </li>
              ))}
            </ul>
          </div>
