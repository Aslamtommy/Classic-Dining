...
managerApi.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Check if the error is due to invalid credentials (e.g., wrong email or password)
    if (
      error.response &&
      error.response.status === 401 &&
      error.response.data.message === "Login failed. Please check your credentials."
    ) {
      console.log("Invalid credentials. Not attempting token refresh.");
      return Promise.reject(error); // Reject the promise to handle the error in the component
    }

    // If the error status is 401 (Unauthorized) and not due to invalid credentials, attempt token refresh
    if (
      error.response &&
      error.response.status === 401 &&
